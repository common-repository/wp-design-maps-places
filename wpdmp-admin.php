...
if ( !function_exists('wpdmp_admin_script') ):
	function wpdmp_admin_script() {		            
		wp_enqueue_style('wpdmp-styles', WPDMP_PLUGIN_URL . 'css/wpdmp.min.css', false, WPDMP_VERSION, "screen");
		...
		wp_enqueue_script('wpdmp-coords', WPDMP_PLUGIN_URL . 'js/wpdmp-coords.min.js', array(), false, true);
		wp_register_script('imagesloaded', WPDMP_PLUGIN_URL . 'imagesloaded-master/jquery.imagesloaded.min.js', array(), false, false);
		wp_enqueue_script('wpdmp_map_manager', WPDMP_PLUGIN_URL . 'js/wpdmp-map-manager.min.js',array('wpdmp-common'), false, false);
		wp_enqueue_script('wpdmp-common', WPDMP_PLUGIN_URL . 'js/wpdmp-common.min.js',array('jquery-ui-dialog','imagesloaded','wpdmp-coords'), false, false);
		//needed for tabs...
		wp_enqueue_style('wpdmp-tabs-styles', WPDMP_PLUGIN_URL . "css/jquery-ui.min.css", false, WPDMP_VERSION, "screen");
		...
		if ($_GET['page'] != 'wpdmp_settings' && strpos($_GET['page'],'wpdmp_')===0){
			...
			wp_enqueue_script('google-map-api', (is_ssl()?'https':'http') . '://maps.google.com/maps/api/js?key='.get_option('wpdmp_gooapikey'), array(), false, false);
			wp_enqueue_script('wpdmp-js3', WPDMP_PLUGIN_URL . 'js/MathLib.min.js', array(), false, true);
			wp_enqueue_media();        
		} 
		if ($_GET['page']=='wpdmp_settings'){
			wp_enqueue_script('wpdmp-settings', WPDMP_PLUGIN_URL . 'js/wpdmp-map-settings.min.js',array('wpdmp-common'), false, false);		
...
if ( !function_exists('wpdmp_front_script') ):
	function wpdmp_front_script() {
	    ...
	    wp_enqueue_script( 'json2' );
		wp_enqueue_style('wpdmp-front-styles', WPDMP_PLUGIN_URL . 'css/wpdmp.min.css', false, WPDMP_VERSION, "screen");	
		...
		wp_enqueue_script('wpdmp-front-js1', WPDMP_PLUGIN_URL . 'js/wpdmp-common.min.js', array('imagesloaded'), false, false);
}
endif;
