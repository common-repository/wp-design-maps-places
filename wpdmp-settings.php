...
if ( !function_exists('wpdmp_settings_view') ):
	...
	<div id="dialog-confirm" title='<?php _e('Remove the language including ALL descriptions in this language?','wp-design-maps-and-places'); ?>' style="width: 100%;">
	...
	function wpdmp_settings_view_inner() {
	   ...
	   <div id='wpdmp_settings'>
			...
			<div id="col-left" style="width:33%">
				...
				<fieldset class="wpdmp-fieldset">
            		<legend><?php _e('Google Maps JavaScript API Key','wp-design-maps-and-places'); ?></legend>
            		<p><?php _e("Generate your own Google Map JavaScript API Key and store here.(Google guide on this: https://developers.google.com/maps/documentation/javascript/get-api-key)",'wp-design-maps-and-places'); ?></p>
            		<p class="submit">
            			<input type='text' size='40' class='wpdmp-setting-input' id='gooapikey_edit' value="<?php echo get_option('wpdmp_gooapikey');?>"/>
                	 	<input type='button' class='button-primary wpdmp-setting-input' tabindex='1' 
                	 		value='<?php _e(' Save Key ','wp-design-maps-and-places'); ?>'
                	 		id='addgookey' name='addgookey' disabled="1"
                	 		onclick="add_gooapikey($jq('#gooapikey_edit').val());"/>
            		</p>
            		<script type="text/javascript">
            		$jq("#gooapikey_edit").change(function() {
            			$jq("#addgookey").prop('disabled', false);
            		});
            		$jq("#gooapikey_edit").keyup(function() {            			
            			$jq("#addgookey").prop('disabled', false);
            		});
            		</script>
				</fieldset>
...
if ( !function_exists('wpdmp_add_gooapikey_callback') ):
function wpdmp_add_gooapikey_callback() {

	$new_key = $_POST['gooapikey'];
	
	update_option('wpdmp_gooapikey', $new_key);
