...
function save_marker(id){
	...
	$jq.post(ajaxurl, data, function(response) {
		...
		if (marker.code == 'ERROR'){
			...
			reload_map(mapid,'map',true,'backend_marker_manager');
...
function reload_map(mapid,id,learn,mode){
	...
	if (id == 'map-front'){	
		...
		if (mode == 'front_end_reload') {
			$jq.post(ajaxurl, data, function(response) {
				$jq('#mapcontainer').empty();
				$jq('#mapcontainer').append(response);
				$jq('#mapcontainer').imagesLoaded(function( $images, $proper, $broken ){
					...
					$jq('#mapoverlay').css('top','0px').width('100%').height('100%');
...
function align_markers(){
	...
	if (get_maptype_from_map()!='freehand'){
		...
		$jq('.ctrl').each(function() {
			...
			var iw = getImageWidth('mapimage');			
			var ih = getImageHeight('mapimage');
			
			var top = pixelToPro((dy-imh),ih);
			var left = pixelToPro((dx-imw),iw);
			
			$jq(this).css('top', top+'%').css('left', left+'%').css('display','block');
	
			//positioning of the popup 
			var offsetx = pixelToPro(parseInt($jq('#mapimage').attr('popupoffsetx'))+dx,iw);
			var offsety = pixelToPro(parseInt($jq('#mapimage').attr('popupoffsety'))+dy,ih);
...
function adjust_popup_position(event) {
